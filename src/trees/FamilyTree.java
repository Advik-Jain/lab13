...
public class FamilyTree
{
	...
	private void addLine(String line) throws TreeException
	{
		...
		String prnt = line.substring(0, colonIndex);// The substring of line that starts at char #0 and ends just before colonIndex. Check the API for class java.util.String, method substring(), if you need guidance.
		...
		String[] childrenArray = childrenString.split(","); // Call childrenString.split(). Check the API for details. The result will be an array of strings, with the separating commas thrown away.
		...
		TreeNode parentNode;
		if (root == null) {
			parentNode = root = new TreeNode( prnt );
			root = parentNode;
	}else
		{
			parentNode = root.getNodeWithName( prnt );////?  There's a method in Node that searches for a named node. 
			//? If the parent node wasn't found, there must have been something wrong in the data file. Throw an exception.
			if (parentNode == null) {
                throw new TreeException( "Parent node '" + prnt + "' not found." );
            }
		}
